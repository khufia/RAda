...
do
    ...
    bash scripts/clip_adapter/base2new_train.sh $dataset ${SEED} ${BS} ${LR} ${EP} ${ALPHA} ${TEMP} ${GPU}
...
do
    ...
    LOG_FILE="${DIR}/log.txt"
    echo "Running $dataset"
    bash scripts/clip_adapter/base2new_test.sh $dataset ${SEED} ${BS} ${LR} ${EP} ${ALPHA} ${TEMP} ${GPU}
    
     # Check if the last command succeededs
    if [ $? -eq 0 ]; then
        echo "Script for $dataset completed successfully."
                # Ensure log file exists before attempting extraction
        if [[ -f "$LOG_FILE" ]]; then
