...
import trainers.ivlp
# import trainers.vpt
import trainers.rada
import trainers.clip_adapter
import trainers.query_key_coop
import trainers.ivl_rada    
import trainers.prompt_rada
import trainers.siglip
import trainers.blip
import trainers.align
import trainers.openclip
import wandb
...
def reset_cfg(cfg, args):
    ...
    if args.layers:
        cfg.TRAINER.RADA.PROJ_LAYERS=args.layers
...
def extend_cfg(cfg):
    ...
    # Config for RADA
    cfg.TRAINER.RADA = CN()
    cfg.TRAINER.RADA.N_CTX = 2  # number of context vectors
    cfg.TRAINER.RADA.CTX_INIT = "a photo of a"  # initialization words
    cfg.TRAINER.RADA.PREC = "fp16"  # fp16, fp32, amp
    cfg.TRAINER.RADA.PROJ_LAYERS = 1
    cfg.TRAINER.RADA.PROMPT_DEPTH = 9 # Max 12, minimum 0, for 1 it will act as shallow MaPLe (J=1)
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new

    # Config for SIGLIP
    cfg.TRAINER.SIGLIP = CN()
    cfg.TRAINER.SIGLIP.N_CTX = 2  # number of context vectors
    cfg.TRAINER.SIGLIP.CTX_INIT = "a photo of a"  # initialization words
    cfg.TRAINER.SIGLIP.PREC = "fp16"  # fp16, fp32, amp
    cfg.TRAINER.SIGLIP.PROJ_LAYERS = 1
    cfg.TRAINER.SIGLIP.PROMPT_DEPTH = 9 # Max 12, minimum 0, for 1 it will act as shallow MaPLe (J=1)
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new

    # Config for BLIP
    cfg.TRAINER.BLIP = CN()
    cfg.TRAINER.BLIP.N_CTX = 2  # number of context vectors
    cfg.TRAINER.BLIP.CTX_INIT = "a photo of a"  # initialization words
    cfg.TRAINER.BLIP.PREC = "fp16"  # fp16, fp32, amp
    cfg.TRAINER.BLIP.PROJ_LAYERS = 1
    cfg.TRAINER.BLIP.PROMPT_DEPTH = 9 # Max 12, minimum 0, for 1 it will act as shallow MaPLe (J=1)
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new

    # Config for ALIGN
    cfg.TRAINER.ALIGN = CN()
    cfg.TRAINER.ALIGN.N_CTX = 2  # number of context vectors
    cfg.TRAINER.ALIGN.CTX_INIT = "a photo of a"  # initialization words
    cfg.TRAINER.ALIGN.PREC = "fp16"  # fp16, fp32, amp
    cfg.TRAINER.ALIGN.PROJ_LAYERS = 1
    cfg.TRAINER.ALIGN.PROMPT_DEPTH = 9 # Max 12, minimum 0, for 1 it will act as shallow MaPLe (J=1)
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new

    # Config for ALIGN
    cfg.TRAINER.OPENCLIP = CN()
    cfg.TRAINER.OPENCLIP.N_CTX = 2  # number of context vectors
    cfg.TRAINER.OPENCLIP.CTX_INIT = "a photo of a"  # initialization words
    cfg.TRAINER.OPENCLIP.PREC = "fp16"  # fp16, fp32, amp
    cfg.TRAINER.OPENCLIP.PROJ_LAYERS = 1
    cfg.TRAINER.OPENCLIP.PROMPT_DEPTH = 9 # Max 12, minimum 0, for 1 it will act as shallow MaPLe (J=1)
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new
    ...
    cfg.TRAINER.PROMPTSRC.N_CTX_VISION = 2  # number of context vectors at the vision branch
    cfg.TRAINER.PROMPTSRC.N_CTX_TEXT = 2  # number of context vectors at the language branch
...
def setup_cfg(args):
    ...
    print(f'config fiule: {args.config_file}')
    if args.config_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bs", type=int, default=4, help="training batch size"
    )
    parser.add_argument(
        "--lr", type=float, default=3.5e-3, help="learning rate"
    )
    parser.add_argument(
        "--ep", type=int, default=5, help="total epochs to train"
    )
    parser.add_argument(
