...
class SigLIP(nn.Module):    
    def __init__(self, cfg, classnames):
        ...
        self.model = AutoModel.from_pretrained("google/siglip-base-patch16-224", torch_dtype=torch.float32, device_map="auto", attn_implementation="sdpa")
    ...
    def forward(self, image, label=None, training=False):
        image = denormalize_siglip(image)
        to_pil = transforms.ToPILImage()
        images = [to_pil(img) for img in image]        
        self.inputs = self.processor(text=self.prompts, images=images, padding="max_length", return_tensors="pt").to("cuda")
        images = self.inputs["pixel_values"]
        with torch.no_grad():
            image_features = self.model.get_image_features(pixel_values=images)#    → shape: (batch_size, vision_hidden_dim)
            text_features = self.model.get_text_features(  #    → shape: (batch_size, text_hidden_dim)
        ...
        logits = logit_scale * logits + logit_bias
        one_hot_labels = torch.zeros(B, K, device=self.device)
        one_hot_labels[torch.arange(B), label] = 1  # Mark matching class indices as 1

        if training:
             # Compute log-sigmoid loss
            loss1 = -torch.mean(torch.log(torch.sigmoid(one_hot_labels * logits)))
            loss2 = self.criterion(mask_final, torch.ones_like(mask_final))
            return (logits, loss1, loss2)
        return logits

@TRAINER_REGISTRY.register()
class SIGLIP(TrainerX):
    ...
    def build_model(self):
        ...
        self.model = SigLIP(cfg, classnames)
...
def denormalize_siglip(tensor):
    """
    Convert a normalized tensor back to raw pixel values in [0, 1] range.
    Assumes shape (3, H, W) and SigLIP normalization.
    """
